...
$pdo = db();
$schemaFile = dirname(__DIR__) . '/database/migrations/004_saas_platform.sql';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'subscription_save') {
            ...
            $stmt->execute([$plan,$status,$cycle,$trial,$end,$tenant]); flash('success','Abonnement aktualisiert.'); saas_redirect('tenants');
        }
...
<a class="<?= $tab==='overview'?'active':'' ?>" href="?tab=overview">Übersicht</a><a class="<?= $tab==='tenants'?'active':'' ?>" href="?tab=tenants">Mandanten</a><a class="<?= $tab==='billing'?'active':'' ?>" href="?tab=billing">Abrechnung</a><a class="<?= $tab==='plans'?'active':'' ?>" href="?tab=plans">Tarife & Add-ons</a>
</nav><small>Angemeldet als</small><div style="padding:0 12px"><?=e($operator['first_name'].' '.$operator['last_name'])?></div><a class="back" style="display:block;margin:24px 12px;color:#fff" href="../public/">← Zur Anwendung</a></aside>
